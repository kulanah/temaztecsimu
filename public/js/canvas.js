...
class Canvas {
  ...
  drawHalo(){
    ...
    context.strokeStyle = 'rgba(255,255,255,' + Math.min(1, 100 / haloR) + ')';
    context.lineWidth = this.calculateHaloLineWidth(haloR);
    context.stroke();
  ...
  drawDiffraction(){
    ...
    this.diffractionAngle = Math.atan2(combinedAstigmatismY, combinedAstigmatismX);
    if(onSpecimen){
      var settings = calculateR1R2Angle(micrographMaterial, 1, 1, 1, 100000, this.diffractionCameraLength, 4);
      for(i = 1; i < 2; i++) {
