...
for(i = 0; i < temLens.length; i++){
	...
	visualArray[i].style.position = "fixed";
	visualArray[i].style.border= "white solid 2px";
	visualArray[i].style.background = "#000";
	visualArray[i].style.color = "#fff";
	visualArray[i].style.width = visualArray[i].name.length * 10 + 8 + "px";
	visualArray[i].style.height = 30 + "px";
	visualArray[i].innerHTML = visualArray[i].name;
	visualArray[i].style.font = "20px Arial";
	visualArray[i].style.textAlign = "left";
}

function drawColumn(){
	var ctx = beamDiag.getContext("2d");
	...
	ctx = beamLabels.getContext("2d");
	ctx.clearRect(-10000,-10000,20000,20000);
	ctx.save();
	ctx = beamDiag.getContext("2d");
	for(var i=0;i<numOfLenses;i++){
		...
		if(temLens[i].kind=="screen"){
			visualArray[i].style.left = offset + 200 + "px";
			visualArray[i].style.top = temLens[i].y() - 4 + "px";
		
		}else{
      if(temLens[i].kind=="source"){
        visualArray[i].style.left = offset + 175 + "px";
        visualArray[i].style.top = temLens[i].y() - 4 + "px";
      }else{
        visualArray[i].style.left = offset + 175 + "px";
        visualArray[i].style.top = temLens[i].y() - 4 + "px";
		...
		beamDiagDiv.appendChild(visualArray[i]);
	}
	ctx.restore();	
